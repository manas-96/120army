...
class _HomeState extends State<Home> with SingleTickerProviderStateMixin {
  ...
  late ValueNotifier<int> notificationCount;
  late ValueNotifier<int> frientreqCount;
  ...
  void initState() {
    ...
    frientreqCount = ValueNotifier<int>(0);
    ...
    PusherService.instance.events.listen((event) {
      ...
      if (event.eventName == 'friend-request-count') {
        // event.data হলো JSON string, parse করতে হবে
        final data = event.data;
        final parsed = jsonDecode(data);
        final count = parsed['count'] ?? 0;
        frientreqCount.value = count;
      }
    });
    ...
    MainService().notificationcountService().then((value) {
      if (value.status == "success") {
        notificationCount.value = value.total;
      }
    });

    MainService().countfriendrequestService().then((value) {
      if (value.status == "success") {
        frientreqCount.value = value.total;
      }
    });
  ...
  void initPusher() async {
    final uid = await SharedPrefUtils.readPrefStr("user_id");
    MainService().pusherConnect(uid);
  }
              ...
              child: TabBar(
                ...
                tabs: [
                  ...
                  ValueListenableBuilder<int>(
                    valueListenable: frientreqCount,
                  ...
                  // ⭐ NEW: Notification tab with badge
                  ValueListenableBuilder<int>(
